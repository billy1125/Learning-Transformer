...
> **適合對象：** 讀完 [`04-gpt-decoder-only.md`](04-gpt-decoder-only.md)（原理與數學）後，想把每一段 nanoGPT 程式碼對回數學式、並準備打開 nanoGPT Notebook 的讀者。
>
> **讀完後你能做什麼：**
> - 對照 `Head` / `MultiHeadAttention` / `FeedForward` / `Block` / `GPT` 類別與 04 的數學節
> - 解釋 Pre-LN 與 Post-LN 在程式上的差異
> - 看懂 nanoGPT 的字元級 tokenizer 與自迴歸生成
> - 說明 KV Cache 為什麼能加速推理
>
> **前置文件：** [`04-gpt-decoder-only.md`](04-gpt-decoder-only.md)（原理與數學）、[`03a-transformer-architecture.md`](03a-transformer-architecture.md)
>
...
> **怎麼讀：** 每節先看程式，再回指 [`04`](04-gpt-decoder-only.md) 對應的數學節（式子在那裡完整推導）。本文只負責「程式如何落實數學」，不重推公式。

---

## 1. `Head`：單頭 Causal Self-Attention

對應數學：[`04`](04-gpt-decoder-only.md) §3（Scaled Dot-Product）＋ §4（Causal Masking）；幾何直覺見 [`03a`](03a-transformer-architecture.md) §1–§4
...
逐行對回 [`04`](04-gpt-decoder-only.md) §3 的式子：`self.key/query/value` 就是投影矩陣 $W_K,W_Q,W_V$；`q @ k.transpose(-2,-1)` 是 $QK^\top$；`* C**-0.5` 是除以 $\sqrt{d_k}$（見 §3 的方差論證）；`masked_fill(...,-inf)` 是 §4 的因果遮罩；`F.softmax` → `@ v` 就是 $A=\text{softmax}(\cdot)$、輸出 $AV$。

**注意：** `C**-0.5` 中 `C = head_size`，即除以 $\sqrt{d_k}$，與 §3 一致。

## 2. `MultiHeadAttention`：多頭 Attention

對應數學：[`04`](04-gpt-decoder-only.md) §5
...
對應數學：[`04`](04-gpt-decoder-only.md) §6
...
對應數學：[`04`](04-gpt-decoder-only.md) §7，注意是 **Pre-LN**（見本文 §7）
...
`nn.Embedding(V, d)` 內部就是一個 $V \times d$ 的矩陣。forward 時輸入 token ID（整數），直接返回對應的列——這就是「查表（Lookup）」，是 $O(1)$ 的索引操作，不是矩陣乘法。（數學形式化見 [`04`](04-gpt-decoder-only.md) §8 與 [`01b`](01b-prerequisites-math.md) §2；它如何被訓練見 [`04`](04-gpt-decoder-only.md) §10）
...
共用的邏輯：「意義接近的詞，embedding 向量接近；接近的向量，預測時也應該分配相近的機率。」實作上共用同一份矩陣，embedding 訓練得更好，同時參數量減少 `vocab_size × n_embd`（GPT-2 規模約 38M 參數；本倉庫的字元級模型約 2.5 萬）。本倉庫的 NB4 為求簡單，未做 Weight Tying，兩個矩陣獨立訓練。（Weight Tying 對梯度的影響見 [`04`](04-gpt-decoder-only.md) §10 的註）

**等一下——這個 `position_embedding` 和 03 講的 PE 是同一件事嗎？**

是同一個目的（注入位置資訊），但做法不同。03a §7.2 推導的是 Sinusoidal PE（固定公式），nanoGPT 用的是 `nn.Embedding` 實作的 **Learned PE**（[`04`](04-gpt-decoder-only.md) §8、03a §7.5）——每個位置一個可訓練向量：
...
| nanoGPT 類別/方法 | 對應數學（[`04`](04-gpt-decoder-only.md)）| 關鍵操作 |
|---|---|---|
...
[`04`](04-gpt-decoder-only.md) §7 描述的原始論文做法是 **Post-LN**：
...
**為什麼 Pre-LN 比較穩定？** 關鍵在殘差主幹 $x + f(\text{LN}(x))$ 保留了一條**完全不經過 LayerNorm 的直通路徑**，梯度可以恆等流過；Post-LN 的 $\text{LN}(x + f(x))$ 則每穿一層都要經過 LayerNorm 的耦合，層數一深梯度尺度就容易失控，因此需要 learning rate warm-up。完整的梯度流推導見 [`04`](04-gpt-decoder-only.md) §7（與 [`05`](05-backpropagation.md) §5.9／§5.10）。
...
1. **為什麼可以 cache？** Causal Mask 保證位置 $t$ 的 K、V 不受未來 token 影響——一旦算出來就永遠不變，可以安心重用（因果性見 [`04`](04-gpt-decoder-only.md) §4）。
...
| 問題 | 對應概念 |
|---|---|
| `Head` 裡的 `self.tril` 遮罩在做什麼？ | Causal Masking（[`04`](04-gpt-decoder-only.md) §4）|
| `C**-0.5` 是什麼？ | $1/\sqrt{d_k}$ 縮放（[`04`](04-gpt-decoder-only.md) §3；幾何見 03a §3.4）|
| `Block` 裡兩個 `x = x + ...` 是什麼結構？ | Residual Connection + Pre-LN（本文 §7、[`04`](04-gpt-decoder-only.md) §7）|
| `lm_head` 輸出的 `(B, T, vocab_size)` 裡，哪個位置是訓練用的目標？ | 每個位置 $i$ 預測 $i+1$（[`04`](04-gpt-decoder-only.md) §9）|
| 為什麼 `generate` 要截取 `idx[:, -block_size:]`？ | Context window 上限（本文 §9）|
| `n_embd=384, n_head=6` → 每個 head 的維度是多少？ | $384/6=64$（[`04`](04-gpt-decoder-only.md) §5）|
| 訓練和生成時 `targets` 的差異？ | 訓練時傳入 targets 算 loss；生成時不傳（[`04`](04-gpt-decoder-only.md) §9、本文 §9）|
...
→ [`04-gpt-decoder-only.md`](04-gpt-decoder-only.md) §10 — 從 loss 到 Embedding 的完整梯度鏈
