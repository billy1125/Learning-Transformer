...
```
01 前置數學        →  02 Attention 直覺  →  03a 架構       →  04a GPT 概念/Pipeline →  05a1 前向數學 →  04b nanoGPT 程式 →  實作
(向量/softmax/梯度)   (QKV 翻譯範例)        (多頭/Block/PE)    (架構差異/Pipeline)      (逐節推導)      (逐行對照數學)      NB1→NB2→NB4
```

> [`04a`](04a-gpt-decoder-only.md) 是 GPT 的**基本概念、架構差異與 Pipeline 總覽**；前向每個模組的數學推導在 [`05a1`](05a1-forward-propagation.md)、反向梯度與數值計算在 [`05b1`](05b1-backward-propagation.md)、逐行程式對照在 [`04b`](04b-nanogpt-walkthrough.md)（每節回指 05a／05b 數學），讀完接 NB4。
