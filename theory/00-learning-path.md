...
| 機率 | 機率分布、期望值 | `03a` §3.2 縮放的統計推導、[`01b`](01b-prerequisites-math.md) |
...
- **SVD** → PCA、LoRA（參數高效微調）
- **數值穩定性** → Softmax overflow、浮點誤差（`03a` §3.2 的實作注意、[`01b`](01b-prerequisites-math.md) §4.3）
...
- [`03b`](03b-transformer-architecture-example.md)：用 $2\times4$ 輸入手算整個 Pre-LN Block（對應 NB1 §13）
