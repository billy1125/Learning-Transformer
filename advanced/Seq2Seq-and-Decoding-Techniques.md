...
它的好處是：避免深層網路在訓練時把早期資訊弄丟；讓梯度比較容易往前傳；也讓模型在需要時可以「保留原本表示」，而不是每一層都被迫完全改寫。白話說，residual 像在每層旁邊搭一條捷徑，資訊不必完全靠中間那段複雜轉換才能傳到下一層。（殘差為何能緩解梯度消失，見 [`theory/03a`](../theory/03a-transformer-architecture.md) §6 與梯度推導 [`theory/05`](../theory/05b1-backward-propagation.md)。）
...
正在生第 3 個位置時，只能看位置 1、2、3，不能看第 4 個。這就是 **causal mask**（也叫 look-ahead mask）的概念，目的是避免訓練時模型偷看未來答案。（它在 nanoGPT 裡怎麼用下三角矩陣把未來位置設成 $-\infty$、以及數值演示，見主線 [`theory/05a`](../theory/05a1-forward-propagation.md) §2。）
