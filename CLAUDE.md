...
```
theory/          ← 理論主線（依序閱讀；00 為前言導讀、06 為 decoder 選讀出口、07 為 encoder 選讀分支、09 為 embedding→RAG 應用出口；04a 為 GPT 基本概念與 Pipeline、05a 為向前傳播數學（原 04a §3–§9）、05b 為向後傳播數學＋數值（原 05 反向推導＋吸收 04a §10）、04b 為 nanoGPT 程式對照續篇；03a-transformer-block-plain 為 03a §6 的白話輔助版、03b1→03b2→03b3 為 03a 的選讀計算案例三階段、03b4 為含位置編碼（P≠0）的選讀對照支線）
...
| `04a-gpt-decoder-only.md` | GPT Decoder-Only **基本概念、架構差異與 Pipeline 總覽**（§1 Encoder-Decoder、§2 為何只要 Decoder、完整 Pipeline 前向＋反向一覽；數學細節分流到 05a／05b）|
| `05a-forward-propagation.md` | GPT **向前傳播數學**（§1 Scaled Dot-Product、§2 Causal Masking、§3 Multi-Head、§4 FFN、§5 LayerNorm/Pre-LN、§6 Embedding/PE、§7 Next-token/CE；由原 04a §3–§9 搬移而來）|
| `04b-nanogpt-walkthrough.md` | GPT Decoder-Only **程式對照**（04a 的選讀續篇）：nanoGPT `Head`/`MultiHeadAttention`/`FeedForward`/`Block`/`GPT` 逐行、Pre-LN vs Post-LN、字元級 Tokenizer、自迴歸生成＋KV Cache、速查清單；每節回指 05a／05b 對應數學節 |
| `05b-backward-propagation.md` | GPT **向後傳播數學＋數值計算**：Self-Attention、LayerNorm 與 Embedding 的完整梯度推導，附 T=2 數值驗證範例與反向傳播五步總覽（吸收原 04a §10）|
...
| `NB3-llm-backpropagation.ipynb` | NumPy 手刻完整反向傳播（含梯度驗證）| 01–03 + 05b |
| `NB4-nanoGPT.ipynb` | 完整 nanoGPT，訓練莎士比亞文本 | 01–03 + 04a/05a/04b |
...
- `04a-gpt-decoder-only.md`（基本概念與 Pipeline）＋ `05a-forward-propagation.md`（向前傳播數學）＋ `05b-backward-propagation.md`（向後傳播數學＋數值）＋ `04b-nanogpt-walkthrough.md`（nanoGPT 程式對照）是關鍵橋接文件，連接理論與 nanoGPT 實作；04b 每節回指 05a／05b 的數學節，改章節號時兩邊要同步；04a 的 Pipeline 總覽節號亦指向 05a（前向）／05b（反向），一併同步
